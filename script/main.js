...
fetchData('posts');
fetchData('todos');
const fetchAllData = async (...types) => {
    const fetchPromises = [...types].map(el => fetch(apiUrl + el).then(res => res.json()));
    const [usersData, postsData] = await Promise.allSettled(fetchPromises);
    const usersValue = usersData.status === 'fulfilled' ? usersData.value : undefined;
    const postsValue = postsData.status === 'fulfilled' ? postsData.value : undefined;
